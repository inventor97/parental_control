...
class _EmailSignInFormBlocBasedState extends State<EmailSignInFormBlocBased> {
  ...
  Future<void> _submit(EmailSignInModel model) async {
    try {
      ...
      await showExceptionAlertDialog(context, title: model.signInFailedText, exception: e);
    }
  }
    ...
    return [
      ...
      model.formType == EmailSignInFormType.register
          ? TextField(
              ...
              decoration: InputDecoration(
                labelText: 'Surname',
                errorText: model.showErrorTextSurname ? model.inValidSurnameErrorText : null,
                enabled: model.isLoading == false,
              ))
          : Opacity(opacity: 0),
      ...
      SizedBox(height: 8.0),
      FormSubmitButton(
        onPressed: model.canSubmitRegister || model.canSubmitSignIn ? () => _submit(model) : () {},
  ...
  Widget _buildPasswordTextField(EmailSignInModel model) {
    return TextField(
      ...
      decoration: InputDecoration(
          labelText: 'Password', errorText: model.showErrorTexPassword ? model.inValidPasswordErrorText : null, enabled: model.isLoading == false),
      obscureText: true,
