...
class _ChildDetailsPageState extends State<ChildDetailsPage> {
  ...
  Widget _buildContentTemporary(BuildContext context, ChildModel? model) {
    if (model != null) {
          ...
          children: [
            ...
            Padding(
              ...
              child: RichText(
                ...
                text: TextSpan(text: "Send notifications to your Child's device", style: TextStyle(color: Colors.indigo, fontSize: 14)),
              ),
            ),
            SizedBox(height: 2),
            Padding(
              ...
              child: RichText(
                textAlign: TextAlign.start,
                text: TextSpan(text: 'Push the button ', style: TextStyle(color: Colors.grey, fontSize: 11)),
              ),
            ),
            SizedBox(
              height: 8,
                      ...
                      onPressed: () async {
                        try {
                          await widget.database.setNotification(
                              ...
                              model);
                          await showAlertDialog(context, title: 'Successful', content: 'Notification sent to ${model.name}', defaultActionText: 'OK');
                          print('Notification sent to device');
                        } on FirebaseException catch (e) {
                          await showExceptionAlertDialog(context, title: 'An error occurred', exception: e);
                        }
                      },
                    ),
                  ),
      ...
      return EmptyContent(title: 'Nothing Here', message: ' Here is the kids details page');
    }
  }
  ...
  Future<void> _confirmDelete(BuildContext context, ChildModel model) async {
    final didConfirmDelete = await showAlertDialog(context,
        title: 'Delete child', content: 'Are you sure you want to delete this child?', defaultActionText: 'Delete', cancelActionText: 'Cancel');
